...
def main():
    ...
    while True:
        ...
        try:
            ...
            _task_start_time = time.time()  # [操作日志] 记录任务开始时间
            
            # [Feature: 智能分流] 如果 Redis payload 中自带了 htmlContent (极速模式)，
            # 直接提取字符串，在本地写入为临时 HTML 并在本流程伪装成文件
            ...
            if html_content:
                ...
                file_path = temp_html_file.name # 掉包原 filePath
            ...
            print(f"📥 接收侧提取任务 -> TaskID: {task_id}, FileCode: {file_code}, File: {source_name}")
