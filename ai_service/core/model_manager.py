...
class ModelManager:
    ...
    def load_model(self):
        ...
        try:
            ...
            if 'CUDAExecutionProvider' in available_providers:
                _cuda_device_id = int(os.getenv('ORT_CUDA_DEVICE_ID', '0'))

                # ── VRAM 状态诊断 ─────────────────────────────────────────────
                # 在加载前打印 VRAM 用量，辅助定位 VRAM 不足 vs TRT 冲突。
                # fp32 BGE-M3 约需 2300MB，fp32 Reranker 约需 500MB，合计 ~2.8GB。
                try:
                    import subprocess as _sp
                    _nv = _sp.run(
                        ['nvidia-smi', f'--id={_cuda_device_id}',
                         '--query-gpu=name,memory.total,memory.free,memory.used',
                         '--format=csv,noheader,nounits'],
                        capture_output=True, text=True, timeout=5
                    )
                    if _nv.returncode == 0:
                        _name, _tot, _free, _used = [x.strip() for x in _nv.stdout.strip().split(',')]
                        _free_mb, _tot_mb = int(_free), int(_tot)
                        print(f"    🖥️  GPU[{_cuda_device_id}] {_name}: "
                              f"total={_tot_mb}MB  used={_used}MB  free={_free_mb}MB")
                        if _free_mb < 2500:
                            print(f"    ⚠️  VRAM 剩余 {_free_mb}MB 可能不足以加载 fp32 BGE-M3 (~2300MB)")
                        # 动态 gpu_mem_limit：使用可用 VRAM 的 70%，避免榨干显存
                        _gpu_mem_limit = int(_free_mb * 0.70) * 1024 * 1024
                    else:
                        _gpu_mem_limit = int(os.getenv('ORT_GPU_MEM_LIMIT_GB', '3')) * 1024 ** 3
                except Exception:
                    _gpu_mem_limit = int(os.getenv('ORT_GPU_MEM_LIMIT_GB', '3')) * 1024 ** 3

                _cuda_opts = {
                    'device_id': _cuda_device_id,
                    # kSameAsRequested：按实际需要分配，不预分配整块显存（防 OOM）
                    # 原 kNextPowerOfTwo 会一次性申请 2^n 块显存，VRAM 紧张时直接失败
                    'arena_extend_strategy': 'kSameAsRequested',
                    'gpu_mem_limit': _gpu_mem_limit,
                    'cudnn_conv_algo_search': 'DEFAULT',
                    'do_copy_in_default_stream': True,
                    # [TRT 冲突修复] 关闭 CUDA Graph 避免 TRT EP 和 CUDA EP 抢占 CUDA 上下文
                    # 根因：TensorRT EP 存在时，CUDA Graph 构建与 TRT 引擎编译会共用同一 stream，
                    #   导致 CUDA context 状态错误，InferenceSession 静默回退 CPU。
                    'enable_cuda_graph': False,
                }
                emb_providers = [('CUDAExecutionProvider', _cuda_opts), 'CPUExecutionProvider']
                self.device = "CUDA"
                # [P1 优化] ORT 1.17+ CUDA EP 已全面支持 ORT_ENABLE_ALL（含 Attention Fusion、
                # LayerNorm Fusion、FastGelu Fusion 等关键算子融合）。
                # 原 ORT_ENABLE_BASIC 是早期规避 CUDA 算子兼容问题的保守设置，现已过时。
                # 回退开关：环境变量 ORT_DISABLE_GRAPH_OPT=1 可立即降级为 ORT_ENABLE_BASIC。
                _use_basic_opt = os.getenv("ORT_DISABLE_GRAPH_OPT", "0") == "1"
                sess_options.graph_optimization_level = (
                    onnxruntime.GraphOptimizationLevel.ORT_ENABLE_BASIC if _use_basic_opt
                    else onnxruntime.GraphOptimizationLevel.ORT_ENABLE_ALL
                )
                # ORT 1.17+ CUDA EP 已支持 enable_mem_pattern，启用后复用已分配显存块，
                # 消除每次推理的 cudaMalloc/cudaFree 开销（实测减少约 10~30ms）。
                sess_options.enable_mem_pattern = True
                # [诊断] 开启 ORT WARNING 日志，打印 CUDA 底层错误（如 cudaMalloc ENOMEM）
                sess_options.log_severity_level = 2   # 0=VERBOSE 1=INFO 2=WARNING 3=ERROR
                sess_options.intra_op_num_threads = 4
                sess_options.inter_op_num_threads = 1
                print(f"🚀 [路径A] CUDA 可用！目标: CUDAExecutionProvider "
                      f"(device={_cuda_device_id} gpu_mem_limit={_gpu_mem_limit//1024//1024}MB)")
            elif 'DmlExecutionProvider' in available_providers:
                emb_providers = ['DmlExecutionProvider', 'CPUExecutionProvider']
                self.device = "DML"
                sess_options.intra_op_num_threads = 4
                print(f"🚀 [路径B] DirectML 可用！目标: DmlExecutionProvider")
            else:
                emb_providers = ['CPUExecutionProvider']
                self.device = "CPU"
                sess_options.intra_op_num_threads = int(os.getenv("AI_CPU_THREADS", 16))
                print(f"⚠️ [路径C] 仅 CPU 可用")

            # --- 加载 Embedding 模型（int8优先，失败回退fp32，保持同一Provider）---
            ...
            # --- 加载 Reranker 模型（int8优先，失败回退fp32，保持同一Provider）---
            print(f"📦 Loading Reranker Model (preferred: {emb_providers[0]})")
            self.reranker = self._load_session_with_fallback(
                self.int8_reranker_path, self.reranker_path,
                emb_providers, sess_options, "Reranker"
            )
            ...
            # --- [关键] 加载后用实际 Provider 更新 self.device，防止"汇报CUDA实跑CPU"的幻觉 ---
            if self.model:
                ...
                if "CUDAExecutionProvider" in actual_providers:
                    ...
                    self.device = "CPU"
                print(f"🎯 [ModelManager] 实际运行设备确认: {self.device} | Providers: {actual_providers}")
    ...
    def rerank(self, query: str, documents: List[str]) -> List[float]:
        """对候选文档进行重排打分"""
        if not documents or self.reranker is None: return []
