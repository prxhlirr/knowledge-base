...
class CoarseChunker:
    ...
    def __init__(self, cfg: dict):
        ...
        # [Phase3] 前言识别正则：公文开头的法律依据/决定引言段落（必须在文档前段出现）
        # 这类段落语义上紧扣文档主题（包含“任职”/“规定”等关键词），
        # 但实际价值低于具体人员信息/条文内容，在语义搜索中常错误排名第一。
        # is_preamble=True 后，RrfFusionStep 会额外给此类 chunk 降权 50%。
        self._IS_PREAMBLE_RE = re.compile(
            r'(根据|按照|依据|依照)[^，。]{0,30}(规定|条例|细则|意见|决定|通知)'
            r'|现将.{0,20}(予以|进行|开展).{0,10}公示'
            r'|经.{2,10}(?:研究决定|会议决定|常委会研究)'
            r'|特此(?:通知|公告|公示|批复)'
            r'|(?:依法|按规定).{0,10}(?:公示|公告)'
        )
    ...
    def _scan(self, markdown_text: str) -> List[Chunk]:
        ...
        for idx, line in enumerate(lines):
            # [P0-A 修复] 空行作为段落边界触发 push_body()。
            if not line.strip():
                push_body()
                # ── [状态机防火墙]：遇到空行，强制复位表格状态，阻断表格跨段落蔓延雪崩 ──
                if is_in_table:
                    push_table()
                    is_in_table = False
                continue
    ...
    def _enrich(self, merged: List[Chunk]) -> List[Chunk]:
        """面包屑织入、overlap 注入（跨章节抑制）、质量分过滤、纯正文快照保存、前言打标"""
        final: List[Chunk] = []
        prev_content = ""
        prev_path    = ""

        # [Phase3] 前言计数器：只对文档前 3 个 body chunk 执行前言识别，避免误杀正文内容。
        _body_chunk_idx      = 0
        _PREAMBLE_CHECK_LIMIT = 3
        ...
        for mc in merged:
            if mc.chunk_type == 'body':
                ...
                # [Phase3] 前言识别：仅对文档前 3 个 body chunk 检测
                # 命中前言正则 → 打标 is_preamble=True，后续 RrfFusionStep 降权 50%
                if _body_chunk_idx < _PREAMBLE_CHECK_LIMIT:
                    _text_check = mc.content[:300]  # 只扫首屏，防 ReDoS
                    if self._IS_PREAMBLE_RE.search(_text_check):
                        mc.is_preamble = True
                _body_chunk_idx += 1
