...
/**
 * 管线节点 1：意图梳理与文本归一�? * 负责�? * - 全角转半角的本地预处�? * - 访问 NLP 网关进行拼音补充和词法规�? * - 路由�?长查询的重写策略（精确查询、短词免重写�? * - 识别 NavigationalBypass 等业务标�? */
@Component
public class QueryNormalizeStep implements SearchPipelineStep {
    ...
    public void execute(SearchContext context) throws Exception {
        ...
        // 1. [精确查询短路] 本地预判：精确查询跳�?Python NLP 归一化（避免 3s 延迟折损�?        // [P2-2 修复] isExactQuery 只调用一次，并用 exactQueryPrecheck 标记初始结果�?        //             避免 NLP 后再�?isExactQuery(normalizedQuery) 覆盖 skipLlmRewrite�?        // �?Bug：L40 �?skipLlmRewrite=true，L74 �?normalizedQuery 重新判断再赋值，
        //         �?NLP 服务修改�?query 格式使其不满足精确判断，L74 会错误改�?false�?        boolean exactQueryPrecheck = isExactQuery(queryText.trim());

        String normalizedQuery;
        String queryPinyin;
        // [Step3 优化] 语义问句（含意图�?+ 长度>6）跳�?Python NLP normalize（消�?~200ms）�?        // 根因：NLP normalize 的价值是拼音补充�?静宁"�?jingnin"）和错别字纠正，
        //       对含"什�?如何/哪些/条件"等意图词的完整语义问句完全无效—�?        //       语义完整，BM25 不依赖拼音，调用 HTTP /api/ai/nlp/normalize 纯属浪费�?        boolean isSemanticQuestion = !exactQueryPrecheck
        ...
        if (exactQueryPrecheck || isSemanticQuestion) {
            ...
            if (exactQueryPrecheck) {
                System.out.println("[QueryNormalizeStep] 精确查询，跳�?Python NLP，使用本地全角→半角预处理�?);
            } else {
                System.out.println("[QueryNormalizeStep] 语义问句（含意图词），跳�?NLP normalize�?200ms）�?);
            }
        } else {
        ...
        // 2. 意图路由与标志位设置
        // [P2-2 修复] skipLlmRewrite 最终结果：精确查询预检 OR NLP 后再次判断（两者取 OR�?        // 场景说明：精确查询预检�?false 时，NLP �?normalizedQuery 若变为精确格式（罕见），
        //           也应标记�?skipLlmRewrite=true，兼顾两种路径�?        boolean skipLlmRewrite = exactQueryPrecheck || isExactQuery(normalizedQuery);
        ...
        // [Navigational Bypass] 如果是小�?字的纯名词，后续跳过重排序，加大BM25权重
    ...
    private boolean isExactQuery(String query) {
        ...
        if (query.contains("�?) || query.contains("�?) || query.contains("�?)
                || Pattern.compile("[0-9]{4}[^0-9]*(号|发|函|令|�?").matcher(query).find()) return true;
