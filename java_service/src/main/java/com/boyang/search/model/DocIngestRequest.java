...
public class DocIngestRequest {
    ...
    /**
     * PDF 扫描件首页跳过开关（默认 false）。
     * 业务场景：带固定格式红头封面的政务扫描件批次，设为 true 后，
     * Python 解析层（PageSkipTransform）会物理裁去 PDF 第一页，仅入库后续正文内容。
     * 仅对 .pdf 格式文件生效，其他格式传 true 时自动忽略，零影响。
     */
    private boolean skipFirstPage = false;

    /**
     * 强制 OCR 开关（默认 false）。
     * 设为 true 后，Python 解析层将强行启用全图 OCR 解析，防止漏字漏扫。
     */
