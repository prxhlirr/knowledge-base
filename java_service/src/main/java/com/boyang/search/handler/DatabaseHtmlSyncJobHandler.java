    ...
    public ReturnT<String> execute(String param) throws Exception {
        ...
        for (Map<String, Object> record : records) {
            ...
            try {
                ...
                // [路由修复] 不再写死 kb_document_v1。
                // AbstractIngestStrategy.buildTaskInfo() 会通过 tag 自动路由到正确索引；
                // 此处未设 tag，将走 DocIndexRoutingService.fallbackIndex (kb_document_official)。
                // 如需路由到特定索引，在此 req.setTag("通知") 即可。
                req.setOwner(author);
