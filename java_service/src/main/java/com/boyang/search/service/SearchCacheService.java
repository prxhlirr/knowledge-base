...
public class SearchCacheService {
    ...
    /**
     * 构建缓存 key。
     * 将 appCode + queryText + topK + 关键 filter 参数拼接后取 SHA-256，避免 key 过长。
     *
     * @param appCode   厂家鉴权码
     * @param queryText 查询词
     * @param topK      返回数量
     * @param filters   过滤参数（只取 data_source 和 user_dept_code 参与 key，用户级权限 user_id 不进缓存）
     */
    public String buildKey(String appCode, String queryText, int topK, Map<String, Object> filters) {
        // user_id 不参与缓存 key，PRIVATE 文档因人而异不适合共享缓存
        // 只取数据源过滤和部门过滤参与 key
        ...
        try {
            ...
            List<Map<String, Object>> cacheable = results.stream()
                .filter(doc -> {
                    ...
                    return !"PRIVATE".equals(vis) && !"GRANT".equals(vis);
                })
