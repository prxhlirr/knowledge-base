...
public class DocIngestService {
    ...
    public String ingest(DocIngestRequest req, String operatorId) throws Exception {
        ...
        // [D4 修复] 在入库前执行三步权限校验（校验 deptCode 合规性、操作者归属、GRANT 用户存在性）
        if (operatorId != null) {
        ...
        if (!taskInfos.isEmpty()) {
            // [D4] 将 uploaderId 和 GRANT 参数随批次传入派发逻辑
            ...
            // 通过 self 代理调用，确保 @Transactional 和 AFTER_COMMIT 事件经过 AOP 代理生效
            self.createAndDispatch(batchId, taskInfos, uploaderId, grantedUsers, grantedRoles);
        }

        log.info("[DocIngest] 批次入库完成 batchId={} ingestType={} files={}", batchId, req.getIngestType(),
                taskInfos.size());
        return batchId;
    }
    ...
    public void createAndDispatch(String batchId, List<Map<String, String>> taskInfos,
            String uploaderId, List<String> grantedUsers, List<String> grantedRoles) {
