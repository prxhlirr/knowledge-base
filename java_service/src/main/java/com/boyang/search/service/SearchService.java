...
public class SearchService {
    ...
    /**
     * 业务功能：单次 HTTP 调用同时获取 Dense（1024维）+ Sparse（token权重字典）双模向量。
     * [P0 优化] 替代原来的 fetchQueryVector + fetchSparseVector 两次串行HTTP+两次ONNX推理方案。
     * 实现原理：调用 Python /api/ai/vector/dual，该接口内部调用 model_manager.encode_dual()，
     * 单次 ONNX forward pass 同时输出 sentence_embedding（dense）和 last_hidden_state 权重（sparse）。
     * 指令前缀逻辑由 Python 侧处理（与 /vector/query 完全对齐）。
     * 降级路径：任何异常返回空 Map，调用方应降级为 fetchQueryVector + fetchSparseVector 分别调用。
     *
     * @param text 原始查询文本（不含 BGE 指令前缀，由 Python 侧自动判断是否添加）
     * @return Map 包含 "dense_vector"(List<Double>) 和 "sparse_vector"(Map<String,Double>)
     */
    @SuppressWarnings("unchecked")
    private Map<String, Object> fetchDualVector(String text) {
        try {
            Map<String, String> payload = new HashMap<>();
            payload.put("text", text);
            String url = aiHost.replace("localhost", "127.0.0.1") + "/api/ai/vector/dual";
            String respJson = restTemplate.postForObject(url, payload, String.class);
            ...
            if (respMap != null && Integer.valueOf(200).equals(respMap.get("code"))) {
                Map<String, Object> data = (Map<String, Object>) respMap.get("data");
                if (data != null) {
                    // 将 sparse_vector 的 Object 值转换为 Double（Jackson 默认反序列化为 Integer/Double）
                    Object rawSparse = data.get("sparse_vector");
                    if (rawSparse instanceof Map) {
                        Map<String, Double> sparseMap = new java.util.LinkedHashMap<>();
                        for (Map.Entry<String, Object> e : ((Map<String, Object>) rawSparse).entrySet()) {
                            if (e.getValue() instanceof Number) {
                                sparseMap.put(e.getKey(), ((Number) e.getValue()).doubleValue());
                            }
                        }
                        data.put("sparse_vector", sparseMap);
                    }
                    System.out.printf("⚡ [DualVector] Text: '%s' | Dense:%dd | Sparse:%dterms | CostMs:%s%n",
                            text.length() > 20 ? text.substring(0, 20) : text,
                            data.get("dense_vector") instanceof java.util.List ? ((java.util.List<?>) data.get("dense_vector")).size() : 0,
                            data.get("sparse_vector") instanceof Map ? ((Map<?, ?>) data.get("sparse_vector")).size() : 0,
                            data.getOrDefault("costMs", "?"));
                    return data;
                }
            }
        } catch (Exception e) {
            System.err.println("[DualVector] fetch failed, degrading to separate calls: " + e.getMessage());
        }
        return new HashMap<>();
    }
    ...
    public List<Map<String, Object>> hybridSearch(String appCode, String queryText, int topK,
        ...
        if (isExactQuery(queryText.trim())) {
            ...
            System.out.println("[ExactQuery Fast-Norm] 精确查询，跳过 Python NLP 归一化，使用本地全角→半角预处理");
        ...
        if (skipLlmRewrite || isShortQuery) {
            // 精确查询 / 短查询（≤15字，含原来被排除的极短查询）：
            // 跳过 LLM 改写，直接 BGE-M3 向量化（~200ms，无任何延迟放大）。
            // [Bug3修复] 原代码 ≤4字走 fetchRewriteAndHyde(query, false) → Python 返回 vector=null
            // → KNN 完全被禁 → 只有 BM25，排序质量极差。
            // 现在：所有 ≤15字查询统一走 BGE-M3 直接编码，进入 KNN+BM25 融合路径。
            //
            // [P0 优化] 原实现调用 fetchQueryVector（dense only）+ 后续 fetchSparseVector（串行）
            // = 两次独立 HTTP + 两次 ONNX 推理 ≈ 400ms。
            // 修复：改为单次 fetchDualVector，Python 侧 encode_dual() 单次 forward pass 同时输出
            // dense + sparse，节省一整次 GPU 推理 + HTTP 往返（预期 -150~220ms）。
            // sparse 结果存入 r["sparse_vector"]，供后续直接复用，跳过 fetchSparseVector HTTP 调用。
            mergedFuture = com.boyang.search.util.AsyncContextUtil.supplyAsync(() -> {
                Map<String, Object> r = new HashMap<>();
                r.put("rewritten_query", queryForMerged);
                if (mightNeedVector) {
                    Map<String, Object> dual = fetchDualVector(queryForMerged);
                    // dense_vector：直接作为 KNN 查询向量
                    r.put("vector", dual.get("dense_vector"));
                    // sparse_vector：缓存到 mergedResult，后续 L1121 直接复用，跳过 fetchSparseVector
                    r.put("sparse_vector", dual.get("sparse_vector"));
                }
                return r;
            });
        } else {
        ...
        if (!skipEmbedding && normalizedQuery != null && normalizedQuery.length() > 2) {
            // [P0 优化] 短查询/精确查询路径已通过 fetchDualVector 在 mergedResult["sparse_vector"] 中
            // 缓存了稀疏向量，直接复用，跳过第二次 HTTP → Python → ONNX 的开销（预期 -150ms）。
            // 长查询路径（走 fetchRewriteAndHyde）mergedResult 中无此字段，降级为独立 fetchSparseVector 调用。
            @SuppressWarnings("unchecked")
            Object cachedSparseObj = mergedResult.get("sparse_vector");
            if (cachedSparseObj instanceof Map) {
                @SuppressWarnings("unchecked")
                Map<String, Double> sparseFromDual = (Map<String, Double>) cachedSparseObj;
                querySparseVector = sparseFromDual.isEmpty() ? null : sparseFromDual;
                System.out.println("⚡ [Sparse] Reused from dual result (0ms extra HTTP). Terms: "
                        + (querySparseVector != null ? querySparseVector.size() : 0));
            } else {
                // 长查询 fallback：独立调用 /vector/sparse
        ...
        if (shouldRunReranker) {
            ...
            // [P0 优化] GPU模式 rerankLimit 从15降至8：Top-8后RRF差距已拉开，节省约-150ms
            int rerankLimit = config.getRerankLimit() != null ? config.getRerankLimit() : 8;
            try {
