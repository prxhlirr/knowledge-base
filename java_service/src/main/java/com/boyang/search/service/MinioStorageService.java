...
public class MinioStorageService {
    ...
    /**
     * 从 MinIO 下载文件输入流（注意：调用者负责在消费完毕后关闭流）。
     * 业务功能：支持直接从对象存储或在 MinIO 离线时的本地降级路径下流式读取文件。
     *
     * @param storagePath MinioStorageService.store() 返回的路径，
     *                    格式为 "knowledge-base/objectKey" 或本地绝对路径
     * @return 文件输入流
     * @throws Exception 当文件不存在或读取失败时抛出异常
     */
    public InputStream downloadStream(String storagePath) throws Exception {
        if (minioClient == null) {
            throw new IllegalStateException("[MinioStorage] MinIO 客户端未初始化，无法下载文件");
        }
        if (storagePath == null || storagePath.isEmpty()) {
            throw new IllegalArgumentException("[MinioStorage] storagePath 不能为空");
        }

        String normalizedPath = storagePath;
        // 如果是完整 URL，提取其中的路径部分并去掉开头的斜杠
        if (storagePath.startsWith("http://") || storagePath.startsWith("https://")) {
            java.net.URI uri = new java.net.URI(storagePath);
            String path = uri.getPath();
            if (path.startsWith("/")) {
        ...
        String prefix = bucketName + "/";
        // 如果不是以 bucketName 为前缀，说明是 MinIO 离线降级时的本地物理路径
        if (!normalizedPath.startsWith(prefix)) {
            java.nio.file.Path localPath = Paths.get(normalizedPath);
            if (Files.exists(localPath)) {
                log.info("[MinioStorage] 检测为本地降级物理路径，执行本地流式读取 path={}", normalizedPath);
                return Files.newInputStream(localPath);
            }
            throw new IllegalArgumentException("[MinioStorage] 无法识别的存储路径，既非 MinIO 对象也非本地物理路径: " + storagePath);
        }

        String objectKey = normalizedPath.substring(prefix.length());
        log.info("[MinioStorage] 开始从 MinIO 下载文件对象 bucket={} object={}", bucketName, objectKey);
