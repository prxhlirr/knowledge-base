...
public interface KbDocGrantsMapper extends BaseMapper<KbDocGrants> {
    ...
    @Select("SELECT * FROM kb_doc_grants " +
            "WHERE source_name = #{sourceName} AND is_active = 1 " +
            "ORDER BY created_at DESC")
    com.baomidou.mybatisplus.core.metadata.IPage<KbDocGrants> findActiveGrantsPage(com.baomidou.mybatisplus.core.metadata.IPage<KbDocGrants> page, @Param("sourceName") String sourceName);
