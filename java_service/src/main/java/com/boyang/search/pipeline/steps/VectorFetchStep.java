...
public class VectorFetchStep implements SearchPipelineStep {
    ...
    public void execute(SearchContext context) throws Exception {
        ...
        if (skipLlmRewrite || isShortQuery) {
            ...
            //   原链路：VectorFetchStep fetchQueryVector(600ms) + EsRecallStep fetchSparseVector(600ms) = 1200ms 串行
            //   优化后：fetchDualVector(600ms) 是 dense/sparse 同时就绪，EsRecallStep 直接从 context 取，节省 ~500ms
