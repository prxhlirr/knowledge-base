...
public class DocRegisterController {
    ...
    public Map<String, Object> registry(@RequestBody Map<String, Object> body,
        ...
        try {
            ...
            log.info("[DocRegistry] outbox WAITING→READY taskId={} sourceName={} v{} hash={}",
                taskId, sourceName, docVersion, fileBaseHash);

            // [Fix] 回调同时充填 kb_doc_register：将占位草稿（PROCESSING + 字段为空）推进到 INDEXED
            // 根因：getNextVersion() 创建了占位记录但字段全为 NULL，必须在此处回填完整免数据。
            // 同时触发 registerDoc() 内部的 2PC ES 状态切换（metadata.is_latest=true）。
            try {
                ...
                log.info("[DocRegistry] kb_doc_register 已充填 taskId={} sourceName={} v{} status=INDEXED",
                    taskId, sourceName, docVersion);
            } catch (Exception regErr) {
                // 非致命：outbox 已就绪 READY，OutboxPoller 仍可激活 ES。仅记录日志，不回滚主流。
                log.error("[DocRegistry] kb_doc_register 充填失败（outbox 已 READY，可手动补充） taskId={} err={}",
